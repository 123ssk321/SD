...
public class Discovery {
	...
	private Map<String, List<URI>> serviceNameByURI;
	...
	Discovery( InetSocketAddress addr, String serviceName, String serviceURI) {
		...
		this.serviceNameByURI = new HashMap<String, List<URI>>(7);
	}
				...
				for(;;) {
					try {
						...
						if (tokens.length == 2) {
							...
							List<URI> serviceURIs = this.serviceNameByURI.get(serviceName);
							if(serviceURIs == null){
								serviceURIs = new LinkedList<URI>();
								serviceURIs.add(new URI(tokens[1]));
								this.serviceNameByURI.put(tokens[0], serviceURIs);
							} else {
								if(!serviceURIs.contains(new URI(tokens[1]))){
									serviceURIs.add(new URI(tokens[1]));
								}
							}
							System.out.print("Known URIs: ");
							for(URI uri : this.knownUrisOf(this.serviceName)){
								System.out.printf("%s ", uri.toString());
							}
							System.out.println();
						}
					} catch (IOException e) {
	...
	public URI[] knownUrisOf(String serviceName) {
		...
		if(serviceURIs != null){
			...
			return serviceURIs.toArray(URIs);
		}
		return null;
